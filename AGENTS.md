...
3. **Database Access**: Access Cloudflare D1 natively via `Astro.locals.runtime?.env?.DB` or `locals.runtime?.env?.DB` (typed via `src/env.d.ts` without any casting). Always maintain in-memory fallback for local tests.
